...
step_monitors() {
  ...
  if command -v systemd-detect-virt &>/dev/null; then
    systemd-detect-virt -q && is_vm=true && vm_type=$(systemd-detect-virt)
  fi
  if $is_vm; then
    info "Virtualization detected: ${B}$vm_type${R}"
  fi
  ...
  # ── Fallback if DRM detection finds nothing ──
  if [[ ${#outputs[@]} -eq 0 ]]; then
    if $is_vm; then
      case "$vm_type" in
        oracle)          outputs=("Virtual1") ;;           # VirtualBox
        qemu|kvm)        outputs=("Virtual-1") ;;          # QEMU/KVM
        vmware)          outputs=("Virtual1") ;;           # VMware
        microsoft)       outputs=("Virtual-1") ;;          # Hyper-V
        *)               warn "Unknown VM type ($vm_type) — using auto-detection" ;;
      esac
    else
      # Physical system: assume laptop with eDP-1 internal display
      info "No DRM outputs detected — assuming laptop with eDP-1"
      outputs=("eDP-1")
    fi
  fi
  ...
  # ── Generate config ──
  if [[ ${#outputs[@]} -eq 0 ]]; then
    echo '-- Auto-detection: no specific monitors configured' > "$ml"
    echo 'monitor=,preferred,auto,1' > "$mc"
    ok "Generated auto-detection monitor config"
  else
